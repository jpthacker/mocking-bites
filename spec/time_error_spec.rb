...
RSpec.describe TimeError do
    it "Returns difference in seconds between server time and the time on this computer" do
        ...
        time_error = TimeError.new(requester_dbl, comp_time_dbl)
        expect(time_error.error).to eq 0.0
        comp_time_dbl = DateTime.parse("2023-03-13T14:14:52.053758+00:00").to_time
        time_error = TimeError.new(requester_dbl, comp_time_dbl)
        expect(time_error.error).to eq 1.0
    end
end